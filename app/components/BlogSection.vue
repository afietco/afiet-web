...
/**
 * Ana sayfadaki taze yazılar bölümü. /api/blog/posts'tan son üç yazıyı çeker;
 * kart görünümü /blog listesindeki kartın sadeleşmiş halidir. Yazı yoksa
 * bölüm HİÇ görünmez: dev/staging'de blog tablosu boş başlar ve bu bir arıza
 * değildir (bkz. CLAUDE.md > Veritabanı).
 */
  ...
  <section v-if="latest.length" aria-labelledby="blog-baslik">
    <div class="mx-auto max-w-6xl px-5 py-24">
      <div class="flex flex-wrap items-end justify-between gap-6">
        ...
        <NuxtLink v-reveal="180" to="/blog" class="btn-ghost shrink-0">{{ homeBlog.cta }}</NuxtLink>
      </div>
          ...
          <div class="flex flex-1 flex-col p-5">
            <p class="text-xs font-bold text-muted">
              ...
              <template v-if="p.readingMinutes">
                · {{ p.readingMinutes }} {{ homeBlog.readingSuffix }}</template
              >
            </p>
            <h3
