...
import type { Accent, IletisimCopy } from '~/data/content'
 ...
 *
 * İki dilde yaşar: /iletisim content.ts > iletisim ile, /en/contact
 * content.en.ts > contactEn ile basar. Kopya şekli IletisimCopy'dir; EN'e
 * özgü alanlar (pul boş metni, gizlilik etiketi, bülten kutusu başlığı)
 * opsiyoneldir ve boşken TR varsayılanına düşer.
 */
...
const privacyTo = computed(() => (en.value ? '/en/privacy' : '/gizlilik'))
const privacyLabel = computed(() => props.copy.privacyLabel ?? 'Gizlilik')
const stampEmptyA = computed(() => props.copy.stampEmptyA ?? 'pul')
const stampEmptyB = computed(() => props.copy.stampEmptyB ?? 'buraya')
const bultenTitle = computed(() => props.copy.bultenTitle ?? 'Sofradan mektubun olsun')
const bultenSub = computed(
  () => props.copy.bultenSub ?? 'Kartpostal tek seferlik; bülten düzenli. İstersen ikisini de gönderelim.',
)
...
const selectedTopic = computed(() => props.copy.topics.find((t) => t.key === topic.value) ?? null)

async function submit() {
  if (!message.value.trim()) {
    note.value = props.copy.missingMessage
    return
  }
  if (!EMAIL_RE.test(email.value.trim())) {
    note.value = props.copy.invalidEmail
    return
  }
  note.value = ''
  state.value = 'sending'
  ...
  <div class="relative overflow-hidden">
    ...
    <div class="relative mx-auto max-w-6xl px-5 py-14 sm:py-20">
      <header class="mx-auto max-w-2xl text-center">
        <p class="text-sm font-extrabold tracking-wide text-brand">{{ copy.eyebrow }}</p>
          ...
          >
            <div class="grid sm:grid-cols-[1.15fr_0.85fr]">
              ...
              <div class="p-7 sm:p-8">
                ...
                <label class="sr-only" for="kart-mesaj">{{ copy.messageLabel }}</label>
                <textarea
                  id="kart-mesaj"
                  v-model="message"
                  rows="7"
      ...
      <div class="mx-auto mt-16 grid max-w-4xl gap-5 sm:grid-cols-2">
        <div class="rounded-3xl border border-line bg-surface p-7 shadow-lift">
          ...
          <p class="mt-1.5 text-sm leading-relaxed font-semibold text-soft">{{ copy.socialSub }}</p>
        ...
        <div class="rounded-3xl border border-line bg-surface p-7 shadow-lift">
          ...
          <p class="mt-1.5 text-sm leading-relaxed font-semibold text-soft">{{ copy.mailBody }}</p>
          <a
      ...
      <div class="mx-auto mt-5 max-w-4xl rounded-3xl border border-line bg-surface p-7 shadow-lift">
        <div class="flex flex-col gap-5 sm:flex-row sm:items-center sm:justify-between">
          ...
          <BultenForm :source="en ? 'en-contact' : 'iletisim'" :lang="lang" class="w-full sm:max-w-sm" />
        </div>
      </div>
    </div>
  </div>
