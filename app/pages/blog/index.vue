  ...
  <section class="mx-auto max-w-3xl px-5 py-14 sm:py-20">
    ...
    <div v-if="posts.length" class="mt-10 grid gap-5">
      <NuxtLink
        ...
        :key="p.slug"
        :to="`/blog/${p.slug}`"
        class="group block rounded-3xl border border-line bg-surface p-6 shadow-lift transition hover:border-brand/40 sm:p-7"
      >
        <p class="text-xs font-bold text-muted">
          <time v-if="p.publishedAt" :datetime="p.publishedAt">{{ fmtDate(p.publishedAt) }}</time>
          <template v-if="p.readingMinutes"> · {{ p.readingMinutes }} {{ blog.readingSuffix }}</template>
        </p>
        <h2
