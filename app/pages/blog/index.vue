  ...
  <section class="mx-auto max-w-6xl px-5 py-14 sm:py-20">
    ...
    <div v-if="posts.length" ref="listTop" class="mt-10 scroll-mt-24">
      ...
      <div class="flex flex-col gap-3 sm:flex-row sm:items-center">
        <label class="sr-only" for="blog-ara">{{ blog.searchLabel }}</label>
        ...
        <label class="sr-only" for="blog-sirala">{{ blog.sortLabel }}</label>
        <div class="relative sm:w-auto">
          ...
          >
            <option value="yeni">{{ blog.sortNew }}</option>
            <option value="eski">{{ blog.sortOld }}</option>
          </select>
          <svg
      ...
      <nav
        v-if="pageCount > 1"
        :aria-label="blog.pagesLabel"
        class="mt-10 flex items-center justify-center gap-2"
      >
        <button
          type="button"
          :aria-label="blog.pagePrev"
          :disabled="page === 1"
